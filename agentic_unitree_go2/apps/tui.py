...
try:
    ...
    from unitree_webrtc_connect.constants import RTC_TOPIC, MCF_CMD
except ImportError:
    print("ERROR: unitree_webrtc_connect not installed.")
    sys.exit(1)

from agentic_unitree_go2.connection import build_webrtc_connection
from agentic_unitree_go2.robot.safety import TuiSafetyConfig, TuiSafetyGovernor
...
_safety_config = TuiSafetyConfig()
_safety_governor = TuiSafetyGovernor(
    _safety_config,
    state_summary=lambda: _state_summary(),
    forward_obstacle_m=lambda: _forward_obstacle_m(),
...
def _motion_blocked_by_state(name: str) -> str | None:
    return _safety_governor.motion_blocked_by_state(name)
...
def _state_summary() -> dict:
    ...
    return {
        ...
        "battery_pct": low.get("bms_state", {}).get("soc", "?"),
        "battery_v": low.get("bms_state", {}).get("voltage", "?"),
...
Examples:
  ...
  uv run go2-tui --ip 10.0.0.200 --no-camera
    ...
    _safety_config.max_step_m = max(0.1, float(args.max_step))
    _safety_config.max_turn_deg = max(5.0, float(args.max_turn))
    _safety_config.allow_acrobatics = bool(args.allow_acrobatics)
