...
from agentic_unitree_go2.robot.safety import McpSafetyConfig, McpSafetyGovernor
...
_safety_config = McpSafetyConfig()
_safety_governor = McpSafetyGovernor(
    _safety_config,
...
def _forward_obstacle_m() -> float:
    return _safety_governor.forward_obstacle_m()
...
async def _connect_robot(args: argparse.Namespace) -> UnitreeWebRTCConnection:
    if args.mock:
        ...
        conn = MockConnection(
            RTC_TOPIC,
            cv2_module=cv2 if _CV2_AVAILABLE else None,
            np_module=np if _CV2_AVAILABLE else None,
            state_tick_s=0.05,
...
async def run_http(args: argparse.Namespace) -> None:
    ...
    print(
        "[go2-mcp] OpenWebUI -> Admin Panel -> Tools -> Add Connection",
        file=sys.stderr,
    )
    print(
...
def main() -> None:
    ...
    _safety_config.max_turn_rad = max(5.0, float(args.max_turn)) * 3.141592653589793 / 180.0
