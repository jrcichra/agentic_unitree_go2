...
async def _video_frame_loop(track) -> None:
    ...
    print("[go2-mcp] Video track received — starting frame loop.", file=sys.stderr)
    frame_count = 0
    while True:
        try:
            ...
            if ok:
                ...
                if frame_count % 30 == 0:
                    print(
                        f"[go2-mcp] Frame {frame_count} captured, shape: {img.shape}",
                        file=sys.stderr,
                    )
        except Exception as e:
...
async def _setup_video_track(conn: UnitreeWebRTCConnection) -> None:
    """
    Start receiving camera frames from the robot.
    conn.video is a WebRTCVideoChannel wrapper. It uses a callback pattern:
    add_track_callback(fn) registers a callback that receives the aiortc track
    ...
    async def on_track(track):
        ...
        _video_track = track
        print(
            f"[go2-mcp] Video track received: {type(track).__name__}", file=sys.stderr
        )
    ...
    print(
        "[go2-mcp] Registered video track callback and enabled video channel.",
        file=sys.stderr,
    )
